...
public class BTree <T, E extends Comparator<T> > {
    ...
    public void splitNode(BTreeNode<T> parent, int pos, BTreeNode<T> node){
        ...
        //将node中后一半关键字转移到newNode中
        while(node.key.size() > M){
            try {
                newNode.key.add(node.key.remove(M));
            } catch (Exception e) {
     ...
     * @param target   插入目标
     */
    public synchronized void insert(BTreeNode<T> baseNode, T target) {
     ...
     * @param target   删除目标
     */
    public synchronized void delete(BTreeNode<T> baseNode, T target){
        if (baseNode == null || baseNode != root) {
     ...
     * @param leftNode 左结点
     * @param node 此结点
     * @param pos 关键字上移到baseNode的pos位置
     */
